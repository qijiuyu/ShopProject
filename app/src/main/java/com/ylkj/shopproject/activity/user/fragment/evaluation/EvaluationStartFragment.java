 ...
 * 待评价
 */
public class EvaluationStartFragment extends BaseFragment implements MyRefreshLayoutListener {
    ...
    public void onRefresh(View view) {
        page=1;
        HttpMethod.commOrder("0", page, HandlerConstant.GET_COMM_ORDER_SUCCESS1, handler);
    }

    @Override
    public void onLoadMore(View view) {
        page++;
        HttpMethod.commOrder("0", page, HandlerConstant.GET_COMM_ORDER_SUCCESS2, handler);
    }
    ...
    private void commOrder(int index) {
        if (null != view && isVisibleToUser && listAll.size() == 0) {
            HttpMethod.commOrder("0", page, index, handler);
        }
    }
