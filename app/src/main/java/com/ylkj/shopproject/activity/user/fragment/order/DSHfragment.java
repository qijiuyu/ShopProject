 ...
 * 待收货订单
 */
public class DSHfragment extends BaseFragment implements MyRefreshLayoutListener {
    ...
    public void onRefresh(View view) {
        page=1;
        HttpMethod.getMyOrder(2,page,HandlerConstant.GET_MY_ORDER_SUCCESS1,handler);
    }

    @Override
    public void onLoadMore(View view) {
        page++;
        HttpMethod.getMyOrder(2,page,HandlerConstant.GET_MY_ORDER_SUCCESS2,handler);
    }
    ...
    private void getMyOrder(int index){
        if(null!=view && isVisibleToUser && listAll.size()==0){
            HttpMethod.getMyOrder(2,page,index,handler);
        }
    }
